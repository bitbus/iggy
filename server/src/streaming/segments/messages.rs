...
impl Segment {
    ...
    pub async fn get_messages(
        ...
        if offset >= first_offset && end_offset <= last_offset {
            let buffered_batches = self.load_messages_from_unsaved_buffer(offset, end_offset);
        }
    ...
    fn load_messages_from_unsaved_buffer(
        ...
        let slice_start = unsaved_messages
            .iter()
            .rposition(|batch| batch.base_offset <= offset)
            .unwrap_or(0);

        unsaved_messages[slice_start..]
            .iter()
            .filter(|batch| batch.get_last_offset() <= end_offset)
            .cloned()
            .collect::<Vec<MessagesBatch>>()
    }
    ...
    async fn load_messages_from_disk(
        ...
        let index_range = self.load_highest_lower_bound_index(start_offset as u32, end_offset as u32).unwrap();
