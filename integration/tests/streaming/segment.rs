...
use server::streaming::models::messages_batch::MessagesBatch;
...
async fn should_persist_and_load_segment_with_messages() {
    ...
    let messages_batch = MessagesBatch::messages_to_batch(0, (messages_count - 1) as u32, messages);
    segment
...
async fn given_all_expired_messages_segment_should_be_expired() {
    ...
    let message_batch = MessagesBatch::messages_to_batch(0, (messages_count - 1) as u32, messages);
    segment
...
async fn given_at_least_one_not_expired_message_segment_should_not_be_expired() {
    ...
    let expired_message_batch = MessagesBatch::messages_to_batch(0, 1, expired_messages);
    let not_expired_message_batch = MessagesBatch::messages_to_batch(1, 2, not_expired_messages);
